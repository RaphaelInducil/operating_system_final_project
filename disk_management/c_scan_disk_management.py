# 6/9/2026
# Disk Management Simulation: C-SCAN Algorithm (Circular SCAN)
...
ALGORITHM_NAME = "C-SCAN Algorithm (Circular SCAN)"
MAX_CYLINDER = 199
MIN_CYLINDER = 0
NUM_REQUESTS = 8
...
def calculate_c_scan(initial_head: int, requests: list[int]) -> dict:
    """Moves to the end of the disk, jumps to 0, and continues upward."""
    pending = requests.copy()
    
    if MAX_CYLINDER not in pending:
        pending.append(MAX_CYLINDER)
    if MIN_CYLINDER not in pending:
        pending.append(MIN_CYLINDER)
    ...
    return {
        "sequence": [initial_head] + [h["to"] for h in history],
...
if __name__ == "__main__":
    ...
    results = calculate_c_scan(initial_head, requests)
